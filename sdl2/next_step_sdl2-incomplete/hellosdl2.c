...
// .x=state->obj.x,.y=state->obj.y

    /* Render blue rectangle */
    SDL_Rect r = {.x=getX(&state->obj),.y=getY(&state->obj),.w=50,.h=50};
    ...
    snprintf(buf, sizeof(buf), "x=%d, y=%d", state->obj.x, state->obj.y);
    ...
    while (1) {
		...
		case SDL_MOUSEBUTTONDOWN:
			if( SDL_GetMouseState(&xMouse,&yMouse) && SDL_BUTTON_LEFT){
				// idea: subtract state from Mouse and check the result.
				// It would really help If I knew how the sqaure is rendered --> where in the square are the x/y coordinates? centre? top left corner?
				// assumtion --> coordinates are in the top left corner due to previous tests.
				// this is the last attempted version. Attempt at implementing the boost subtask failed.
				if((xMouse <= state.obj.x+15 || xMouse >= state.obj.x) && (yMouse <= state.obj.y+15 || yMouse >= state.obj.y)){
					boost(&state.obj);
					xMouse = 0;
					yMouse = 0;
				}
